...
def transformer(U, flow, name='SpatialTransformer', **kwargs):
    ...
    theta: float
        The output of the
        localisation network should be [batch_size, 6].
    ...
    def _transform(flow, input_dim):
        with tf.variable_scope('_transform'):
            batch_size = tf.shape(input_dim)[0]
            height = tf.shape(input_dim)[1]
            width = tf.shape(input_dim)[2]
            num_channels = tf.shape(input_dim)[3]
            flow = tf.reshape(flow, [batch_size, 2, -1])
            flow = tf.cast(flow, 'float32')
            ...
            # shape: (batch_size, 2, num_pixels)
            grid = tf.reshape(grid, tf.stack([batch_size, 2, -1]))

            # (u_t, v_t) + (delta_u, delta_v) -> (u_s, v_s)
            T_g = grid + flow
